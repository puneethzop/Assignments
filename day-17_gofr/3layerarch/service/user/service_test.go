...
func TestCreateUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func() {
				mockStore.EXPECT().CreateUser(ctx, models.User{Name: "Alice"}).Return(nil)
			},
			wantErr: nil,
		},
		{
	...
	for _, test := range tests {
		...
		err := svc.CreateUser(ctx, test.input)
...
func TestGetUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			inputID: 1,
			setup: func() {
				mockStore.EXPECT().GetUser(ctx, 1).Return(models.User{ID: 1, Name: "Alice"}, nil)
			},
		...
		},
		{
			...
			inputID: 2,
			setup: func() {
				mockStore.EXPECT().GetUser(ctx, 2).Return(models.User{}, sql.ErrNoRows)
			},
		...
		},
		{
			...
			inputID: 3,
			setup: func() {
				mockStore.EXPECT().GetUser(ctx, 3).Return(models.User{}, errors.New("some db error"))
			},
	...
	for _, test := range tests {
		...
		got, err := svc.GetUser(ctx, test.inputID)
