...
func TestCreateTask(t *testing.T) {
	...
	for _, test := range tests {
		if test.ifMock {
			if test.userErr != nil {
				mockUserService.EXPECT().GetUser(ctx, test.task.UserID).Return(models.User{}, test.userErr)
			} else {
				mockUserService.EXPECT().GetUser(ctx, test.task.UserID).Return(models.User{ID: test.task.UserID}, nil)
				mockTaskStore.EXPECT().CreateTask(ctx, test.task).Return(test.storeErr)
			}
		}

		err := svc.CreateTask(ctx, test.task)
...
func TestGetTask(t *testing.T) {
	...
	for _, test := range tests {
		if test.taskID > 0 {
			mockTaskStore.EXPECT().GetTask(ctx, test.taskID).Return(test.task, test.err)
		}

		got, err := svc.GetTask(ctx, test.taskID)
		if !errorsEqual(err, test.wantErr) {
			t.Errorf("%v: expected error %v, got %v", test.desc, test.wantErr, err)
		}
...
func TestViewTasks(t *testing.T) {
	...
	for _, test := range tests {
		mockTaskStore.EXPECT().ViewTasks(ctx).Return(test.mockData, test.mockErr)

		got, err := svc.ViewTasks(ctx)
		if !errorsEqual(err, test.wantErr) {
			t.Errorf("%v: expected error %v, got %v", test.desc, test.wantErr, err)
		}
		if !reflect.DeepEqual(got, test.want) {
			t.Errorf("%v: expected tasks %v, got %v", test.desc, test.want, got)
		}
	}
}
...
func TestUpdateTask(t *testing.T) {
	...
	for _, test := range tests {
		if test.taskID > 0 {
			mockTaskStore.EXPECT().GetTask(ctx, test.taskID).Return(models.Task{}, test.getErr)
			if test.getErr == nil {
				mockTaskStore.EXPECT().UpdateTask(ctx, test.taskID).Return(test.updateErr)
			}
		}

		err := svc.UpdateTask(ctx, test.taskID)
		if !errorsEqual(err, test.wantErr) {
			t.Errorf("%v: expected error %v, got %v", test.desc, test.wantErr, err)
		}
	}
...
func TestDeleteTask(t *testing.T) {
	...
	for _, test := range tests {
		if test.taskID > 0 {
			mockTaskStore.EXPECT().GetTask(ctx, test.taskID).Return(models.Task{}, test.getErr)
			if test.getErr == nil {
				mockTaskStore.EXPECT().DeleteTask(ctx, test.taskID).Return(test.deleteErr)
			}
		}

		err := svc.DeleteTask(ctx, test.taskID)
		if !errorsEqual(err, test.wantErr) {
			t.Errorf("%v: expected error %v, got %v", test.desc, test.wantErr, err)
		}
	}
